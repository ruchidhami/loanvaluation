...
!function ($) {
  var calenderData = {
    ...
    minBsYear: 2e3,
    maxBsYear: 2090,
    minAdDateEqBsDate: {ad: {year: 1943, month: 3, date: 14}, bs: {year: 2e3, month: 1, date: 1}}
  }, validationFunctions = {
  ...
  $.extend(calenderFunctions, {
    getNepaliNumber: function (number) {
      ...
      var daysNumFromMinBsYear = calenderFunctions.getTotalDaysNumFromMinBsYear(bsYear, bsMonth, bsDate), adDate = new Date(calenderData.minAdDateEqBsDate.ad.year, calenderData.minAdDateEqBsDate.ad.month, calenderData.minAdDateEqBsDate.ad.date - 1);
      adDate.setDate(adDate.getDate() + daysNumFromMinBsYear);
      var bsMonthFirstAdDate = calenderFunctions.getAdDateByBsDate(bsYear, bsMonth, 1), bsMonthDays = calenderFunctions.getBsMonthDays(bsYear, bsMonth);
      bsDate = bsDate > bsMonthDays ? bsMonthDays : bsDate;
      var eqAdDate = calenderFunctions.getAdDateByBsDate(bsYear, bsMonth, bsDate), weekDay = eqAdDate.getDay(), formattedDate = calenderFunctions.bsDateFormat(dateFormatPattern, bsYear, bsMonth, bsDate, weekDay);
      return {
        bsYear: bsYear,
        bsMonth: bsMonth,
        bsDate: bsDate,
      ...
      for (var bsMonthData = calenderData.extractedBsMonthData[bsMonth], i = 0; i < bsMonthData.length; i++)if (0 !== bsMonthData[i]) {
      ...
      for (var yearCount = 0, totalYears = bsYear + 1 - calenderData.minBsYear, bsMonthData = calenderData.extractedBsMonthData[bsMonth], i = 0; i < bsMonthData.length; i++)if (0 !== bsMonthData[i]) {
        var bsMonthUpperDaysIndex = i % 2;
      ...
      var bsYear = adYear + 57, bsMonth = (adMonth + 9) % 12, bsDate = 1;
      if (3 > adMonth)bsYear -= 1; else if (3 == adMonth) {
      ...
      for (var diffTextNum = 0, extractedFormattedBsDate = {
        ...
        var valueOf = dateFormat.substring(i, i + 2), endChar = dateFormat.charAt(i + 2), tempText = dateFormattedText.substring(i + diffTextNum), endIndex = "" !== endChar ? tempText.indexOf(endChar) : tempText.length, value = tempText.substring(0, endIndex);
    ...
    var datePickerPlugin = {
      ...
      addEventHandler: function ($element, $nepaliDatePicker) {
        $element.click(function () {
          ...
          return datePickerPlugin.renderNextMonthCalender($nepaliDatePicker), datePickerPlugin.triggerChangeEvent($element, $nepaliDatePicker, preCalenderData), $nepaliDatePicker.show(), !1
          ...
          return datePickerPlugin.triggerChangeEvent($element, $nepaliDatePicker, preCalenderData), $nepaliDatePicker.show(), !1
          ...
          if (!$(this).hasClass("disable")) {
            var datePickerData = $nepaliDatePicker.data(), bsYear = datePickerData.bsYear, bsMonth = datePickerData.bsMonth, preDate = datePickerData.bsDate, bsDate = $(this).data("date"), weekDay = $(this).data("weekday"), dateText = calenderFunctions.bsDateFormat(datePickerPlugin.options.dateFormat, bsYear, bsMonth, bsDate, weekDay);
            return $element.val(dateText), datePickerPlugin.setCalenderDate($nepaliDatePicker, bsYear, bsMonth, bsDate), datePickerPlugin.renderMonthCalender($nepaliDatePicker), datePickerPlugin.eventFire($element, $nepaliDatePicker, "select"), preDate != bsDate && datePickerPlugin.eventFire($element, $nepaliDatePicker, "dateChange"), datePickerPlugin.options.closeOnDateSelect ? $nepaliDatePicker.hide() : $nepaliDatePicker.show(), !1
          }
          ...
          var preCalenderData = {
            ...
            bsMonth: $nepaliDatePicker.data().bsMonth,
            bsDate: $nepaliDatePicker.data().bsDate
          }, bsMonth = $nepaliDatePicker.find(".month-drop-down").data("value"), bsYear = $nepaliDatePicker.find(".year-drop-down").data("value"), bsDate = preCalenderData.bsDate;
      ...
      getCustomSelectOption: function (datas, activeValue) {
        var $dropDown = $('<div class="drop-down-content" data-value="' + activeValue + '">'), $dropDownWrapper = $('<div class="option-wrapper">'), $ul = $("<ul>");
      ...
      getCalenderBody: function ($nepaliDatePicker) {
        ...
        for (var calenderBody = $("<tbody>"), i = 0; weekCoverInMonth > i; i++) {
          for (var tableRow = $("<tr>"), k = 1; 7 >= k; k++) {
            var calenderDate = 7 * i + k - datePickerData.bsMonthFirstAdDate.getDay(), isCurrentMonthDate = !0;
      ...
      renderCurrentMonthCalender: function ($nepaliDatePicker) {
        var currentDate = new Date, currentBsDate = calenderFunctions.getBsDateByAdDate(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()), bsYear = currentBsDate.bsYear, bsMonth = currentBsDate.bsMonth, bsDate = currentBsDate.bsDate;
        datePickerPlugin.setCalenderDate($nepaliDatePicker, bsYear, bsMonth, bsDate), datePickerPlugin.renderMonthCalender($nepaliDatePicker)
      },
      renderPreviousMonthCalender: function ($nepaliDatePicker) {
        var datePickerData = $nepaliDatePicker.data(), prevMonth = datePickerData.bsMonth - 1 >= 0 ? datePickerData.bsMonth - 1 : 11, prevYear = 11 != prevMonth ? datePickerData.bsYear : datePickerData.bsYear - 1, prevDate = datePickerData.bsDate;
      ...
      renderNextMonthCalender: function ($nepaliDatePicker) {
        var datePickerData = $nepaliDatePicker.data(), nextMonth = datePickerData.bsMonth + 1 <= 11 ? datePickerData.bsMonth + 1 : 0, nextYear = 0 !== nextMonth ? datePickerData.bsYear : datePickerData.bsYear + 1, nextDate = datePickerData.bsDate;
